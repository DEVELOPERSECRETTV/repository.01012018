...
import urllib,urllib2,re,os,threading,datetime,time,base64,xbmc,xbmcplugin,xbmcgui,xbmcaddon,xbmcvfs,plugintools
...
addonArt            = os.path.join(addonPath,'resources/art/livetv')
addonIcon           = os.path.join(addonArt,'livetv-logo.png')
addonFanart         = os.path.join(addonArt,'livetv-wall.jpg')
addonNext           = os.path.join(addonArt,'videos_next.png')
...
class main:
    def __init__(self):
        ...
        elif action == 'play':                      plugintools.play_resolved_url(url)
...
class index:
    def infoDialog(self, str, header=addonName):
        try: xbmcgui.Dialog().notification(header, str, thumbnail, 3000, sound=False)
        except: xbmc.executebuiltin("Notification(%s,%s, 3000, %s)" % (header, str, thumbnail))
    ...
    def cache_clear_list(self):
        try:
            ...
            index().infoDialog("Cache cleared")
        except:
            pass
    ...
    def rootList(self, rootList):
        ...
        for i in rootList:
            try:
                ...
                item.setInfo(type="Video", infoLabels={"title": name, "plot": 'LiveTV on Demand'})
    ...
    def videoList(self, videoList):
        ...
        for i in videoList:
            try:
                ...
                if plot == '0': meta.update({'plot': 'LiveTV on Demand'})
                meta = dict((k,v) for k, v in meta.iteritems() if not v == '0')

                u = '%s?action=videos_parts&url=%s&meta=%s' % (sys.argv[0], sysurl, sysmeta)

                cm = []
                cm.append(('Play', 'RunPlugin(%s?action=item_play)' % (sys.argv[0])))
                cm.append(('Queue item', 'RunPlugin(%s?action=item_queue)' % (sys.argv[0])))
                cm.append(('Now playing...', 'RunPlugin(%s?action=playlist_open)' % (sys.argv[0])))
                cm.append(('Set Videos view', 'RunPlugin(%s?action=view_videos)' % (sys.argv[0])))
                cm.append(('Add-on settings', 'RunPlugin(%s?action=settings_open)' % (sys.argv[0])))
        ...
        try:
            ...
            name, url, image = 'Next Page', next, addonNext
...
class contextMenu:
    ...
    def view(self, content):
        try:
            ...
            index().infoDialog('%s%s%s' % ("", viewName, 'is on by default'))
        except:
            return
...
class link:
    def __init__(self):
        ...
        self.livetv_nba = 'http://livetv.sx/en/videotourney/300'
        self.livetv_nhl = 'http://livetv.sx/en/videotourney/300'
        self.livetv_nba_teams = 'http://livetv.sx/en/leagueresults/300/'
        self.livetv_nhl_teams = 'http://livetv.sx/en/leagueresults/300/'
...
class resolver:
    def run(self, url):
        try:
            ...
            index().infoDialog('No stream available')
            return
